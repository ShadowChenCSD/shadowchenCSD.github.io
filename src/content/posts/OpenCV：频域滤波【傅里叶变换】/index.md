...
​		**对图像进行傅里叶变换后，得到的是一个复数数组**。为了显示方便，**需要将它们的值调整到[0, 255]的灰度空间内**。可以使用`20 * np.log(np.abs(频谱值))`进行。
...
​		**对图像进行傅里叶变换后，得到的是一个复数数组**。为了显示方便，**需要将它们的值调整到[0, 255]的灰度空间内**。可以使用`20 * np.log(幅度值)`进行
